    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function setupCanvases() {
            ...
            if (reused) {
                ...
                if(placeholder.data("plot") != undefined)
        ...
        plot.forceMouseMove = function() {
            eventHolder.mousemove();
        };
        ...
        function drawSeriesBars(series) {
            function plotBars(datapoints, barLeft, barRight, offset, fillStyleCallback, axisx, axisy) {
                ...
                for (var i = 0; i < points.length; i += ps) {
                    ...
                    if (Object.prototype.toString.call(points[i]) === '[object Array]') {
                        if(series.bars.barType == 'continued'){
                            barRight = points[i][1]-points[i][0];
                            drawBar(points[i][0], points[i + 1], points[i + 2], barLeft, barRight, offset, fillStyleCallback, axisx, axisy, ctx, series.bars.horizontal, series.bars.lineWidth);
                        }

                    } else {
                        if(series.bars.barType == 'continued'){
                            if(ps==4) {
                                barRight = points[i+3]-points[i];

                            } else {
                                if(i != 0){
                                    barRight = points[i- ps] - points[i];
                                }else{
                                    barRight = points[i] - points[i + ps];
                                }
                            }
                        }
            ...
            for (i = series.length - 1; i >= 0; --i) {
                ...
                if (s.bars.show && !item) { // no other point can be nearby
                    ...
                    for (j = 0; j < points.length; j += ps) {
                        ...
                        if(s.bars.barType == 'continued'){
                            if(ps==4) {
                                barRight = points[j+3] - points[j];
                            } else {
                                if(i != 0){
                                    barRight = points[j- ps] - points[j];
                                }else{
                                    barRight = points[j] - points[j + ps];
                                }
                            }
                        }
                        if (x == null)
                        ...
                        if (series[i].bars.horizontal ? 
                            ...
                            if(series[i].bars.barType == 'continued') {
                                if(lowestY==-1 || lowestY>y) {
                                    lowestY=y;
                                    item = [i, j / ps];
                                }
                            } else item = [i, j / ps];
