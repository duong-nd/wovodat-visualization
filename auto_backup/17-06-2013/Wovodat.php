...
// dd_edm
                    ...
                    if ($value && mysql_num_rows($value)) {
                        while ($row = mysql_fetch_assoc($value)) {
                            $id1=$row['ds_id1'];
                            $id2=$row['ds_id2'];
                            $codeQuery = mysql_query("select ds_code from ds where ds_id = $id1");
                            $code1 = mysql_fetch_array($codeQuery);
                            $code1 = $code1[0];
                            $codeQuery = mysql_query("select ds_code from ds where ds_id = $id2");
                            $code2 = mysql_fetch_array($codeQuery);
                            $code2 = $code2[0];
                            $newcode=$code1."___".$code2;
...
// dd_ang
                    ...
                    while($row = mysql_fetch_assoc($value)) {
                        ...
                        $newcode=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$id"));
                        $newcode=$newcode[0];
                        if($ref1) {
                            $tmp=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$ref1"));
                            $tmp=$tmp[0];
                            $newcode.="___".$tmp;
                        }
                        if($ref2) {
                            $tmp=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$ref2"));
                            $tmp=$tmp[0];
                            $newcode.="___".$tmp;
                        }
                        
                        if(!$used_dd_ang[$newcode]) {
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Hort1;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Hort2;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Vert1;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Vert2;";
                            $used_dd_and[$newcode]=true;
                        }
                    }
// dd_gps
...
// dd_lev
                    ...
                    while($row = mysql_fetch_assoc($alue)) {
                        ...
                        if($ref) {
                            $tmp=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$ref"));
                            $newcode=$tmp[0];
                        }

                        if($id1) {
                            $tmp=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$id1"));
                            $tmp=$tmp[0];
                        ...
                        if($id2) {
                            $tmp=mysql_fetch_array(mysql_query("select ds_code from ds where ds_id=$id2"));
                            $tmp=$tmp[0];
                            if($newcode!="") $newcode.="___";
                            $newcode.=$tmp;
                        }
...
where a.ds_code = '$code' and a.ds_id = b.ds_id and (c.max - UNIX_TIMESTAMP(b.dd_tlt_time)) mod 43200 < 600  and a.ds_pubdate <= now() and b.dd_tlt_pubdate <= now()  order by b.dd_tlt_time desc");
                    ...
                    case 'EDM':
                        $arr=split('___',$code);
                        $code1=$arr[0];
                        $id1=mysql_fetch_array(mysql_query("select ds_id from ds where ds_code='$code1'"));
                        $id1=$id1[0];
                        $code2=$arr[1];
                        $id2=mysql_fetch_array(mysql_query("select ds_id from ds where ds_code='$code2'"));
                        $id2=$id2[0];
                        //echo $id1;
                        $result = mysql_query("select b.dd_edm_time, b.dd_edm_line$cc from dd_edm b where b.ds_id1 = $id1 and b.ds_id2=$id2 and b.dd_edm_pubdate <= now()  order by b.dd_edm_time desc");
                        break;
                    case 'Angle':
                        switch ($component) {
                            case 'Hort1':
                    ...
                    case 'GPS':
                        ...
                        for($i=0;$i<=2;$i++) {
                            ...
                            if($arr[$i]) {
                                $arr[$i] = mysql_fetch_array(mysql_query("select ds_id from ds where ds_code='$arr[$i]'"));
                                $arr[$i] = $arr[$i][0];
                                $arr[$i]=" = '".$arr[$i]."'";
                            }
                            else $arr[$i]=' is null ';
                        ...
                        switch ($component) {
                            case 'Lat':
                                $result = mysql_query("select b.dd_gps_time, b.dd_gps_lat$cc , b.dd_gps_lon from ds a, dd_gps b where b.ds_id $arr[0] and b.ds_id_ref1 $arr[1] and b.ds_id_ref2 $arr[2] and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                ...
                                break;
                            case 'Lon':
                                $result = mysql_query("select b.dd_gps_time, b.dd_gps_lon$cc , b.dd_gps_lat from ds a, dd_gps b where b.ds_id $arr[0] and b.ds_id_ref1 $arr[1] and b.ds_id_ref2 $arr[2] and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                $additionalData = mysql_query("select min(b.dd_gps_time), b.dd_gps_lat, b.dd_gps_lon from ds a, dd_gps b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gps_pubdate <= now()");
                                break;
                            case 'Elev':
                                $result = mysql_query("select b.dd_gps_time, b.dd_gps_elev$cc from ds a, dd_gps b where  b.ds_id $arr[0] and b.ds_id_ref1 $arr[1] and b.ds_id_ref2 $arr[2] and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                break;
                        }
                        break;
                    case 'GPSVector':
                    ...
                    case 'Leveling':
                        ...
                        $query = "select b.dd_lev_time, b.dd_lev_delev$cc from ds a, dd_lev b where b.ds_ref $arr[0] and b.ds_id1 $arr[1] and b.ds_id2 $arr[2] and b.dd_lev_pubdate <= now() order by b.dd_lev_time desc";
