...
class Wovodat {
    ...
    /*
     * Return the list of Earthquake Type
     */
...
// td
    ...
    private function idToCode($id) {
        $query = "select ds_code from ds where ds_id=$id";
        $result = mysql_query($query);
        $arr = mysql_fetch_array($result);
        return $arr['ds_code'];
    }

    private function codeToId($code) {
        $query = "select ds_id from ds where ds_code='$code'";
        $result = mysql_query($query);
        $arr = mysql_fetch_array($result);
        return $arr['ds_id'];
    }
    
    private function getNewCode($codeArr) {
        $newcode="";
        foreach($codeArr as $key => $value) {
            if($newcode!="") $newcode.="___";
            if(!$value) $newcode.=$key.'-'."null";
            else $newcode.=$key.'-'.$this->idToCode($value);
        }
        return $newcode;
    }

    private function getOldCode($newcode) {
    ...
    private function getAttribute($newcode) {
        $arr = $this->getOldCode($newcode);
...
// dd_tlt
                    ...
                    if ($value && mysql_num_rows($value)) {
                        echo "Deformation&ElectronicTilt&$code&$temp[1]&$temp[2]&Tilt1&microradians;";
                        echo "Deformation&ElectronicTilt&$code&$temp[1]&$temp[2]&Tilt2&microradians;";
                        echo "Deformation&ElectronicTilt&$code&$temp[1]&$temp[2]&Temp&celsiusdegree;";
                    }
// dd_tlv
                    $value = mysql_query("select b.ds_id from ds a, dd_tlv b where a.ds_code = '$code' and a.ds_id = b.ds_id limit 0 , 1");
                    if ($value && mysql_num_rows($value)) {
                        echo "Deformation&TiltVector&$code&$temp[1]&$temp[2]&mag&microradians;";
                        echo "Deformation&TiltVector&$code&$temp[1]&$temp[2]&azi&degree;";
                    }
// dd_str
                    $value = mysql_query("select b.ds_id from ds a, dd_str b where a.ds_code = '$code' and a.ds_id = b.ds_id limit 0 , 1");

                    if ($value && mysql_num_rows($value)) {
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&comp1&microstrain;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&comp2&microstrain;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&comp3&microstrain;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&comp4&microstrain;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&vdstr;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&sstr_ax1;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&azi_ax1;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&sstr_ax2;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&azi_ax2;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&sstr_ax3;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&azi_ax3;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&pmax;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&pmin;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&pmax_die;";
                        echo "Deformation&Strain&$code&$temp[1]&$temp[2]&pmin_die;";
                    }
// dd_edm
                    ...
                    while ($row = mysql_fetch_assoc($value)) {
                        ...
                        $newcode = $this->getNewCode($parameter);
                        if(!$used_dd_edm[$newcode]) {
                            $used_dd_edm[$newcode]=true;
                            echo "Deformation&EDM&$newcode&$temp[1]&$temp[2]&undefined&m;";
                        }
                    }
// dd_ang
                    ...
                    while($row = mysql_fetch_assoc($value)) {
                        ...
                        if(!$used_dd_ang[$newcode]) {
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Hort1&degree;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Hort2&degree;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Vert1&degree;";
                            echo "Deformation&Angle&$newcode&$temp[1]&$temp[2]&Vert2&degree;";
...
// dd_gps
                    ...
                    while($row = mysql_fetch_assoc($value)) {
                        ...
                        if(!$used_dd_gps[$newcode]) {
                            echo "Deformation&GPS&$newcode&$temp[1]&$temp[2]&Lat&degree;";
                            echo "Deformation&GPS&$newcode&$temp[1]&$temp[2]&Lon&degree;";
                            echo "Deformation&GPS&$newcode&$temp[1]&$temp[2]&Elev&m;";
...
// dd_gpv
                    ...
                    if ($value && mysql_num_rows($value)) {
                        echo "Deformation&GPSVector&$code&$temp[1]&$temp[2]&dmag&mm;";
                        echo "Deformation&GPSVector&$code&$temp[1]&$temp[2]&daz&degree;";
                        echo "Deformation&GPSVector&$code&$temp[1]&$temp[2]&NS&mm;";
                        echo "Deformation&GPSVector&$code&$temp[1]&$temp[2]&EW&mm;";
                        echo "Deformation&GPSVector&$code&$temp[1]&$temp[2]&Z&mm;";
                    }
// dd_lev
                    ...
                    while($row = mysql_fetch_assoc($alue)) {
                        ...
                        if(!$used_dd_lev[$newcode]) {
                            ...
                            echo "Deformation&Leveling&$newcode&$temp[1]&$temp[2]&m;";
                        }
                    }
// dd_sar: no station? , how to display the data
// use volcano id instead
    ...
    public function getStationData($type, $table, $code, $component, $referenceTime) {
        ...
        switch ($type) {
            case 'seismic':
                switch ($table) {
                    case 'Interval':
                        $result = mysql_query("select b.sd_ivl_stime, b.sd_ivl_nrec $cc,b.sd_ivl_eqtype from ss a, sd_ivl b where a.ss_code = '$code' and a.ss_id = b.ss_id and a.ss_pubdate <= now() and b.sd_ivl_pubdate <= now() order by b.sd_ivl_stime desc");
                        break;
                    case 'RSAM':
...
where a.ds_code = '$code' and a.ds_id = b.ds_id and (c.max - UNIX_TIMESTAMP(b.dd_tlt_time)) mod 43200 < 600  and a.ds_pubdate <= now() and b.dd_tlt_pubdate <= now()  order by b.dd_tlt_time desc");
                    ...
                    case 'TiltVector':
                        ...
                        $result = mysql_query("select b.dd_tlv_stime,b.dd_tlv_etime, b.$attribute $cc from ds a, dd_tlv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_tlv_pubdate <= now() order by dd_tlv_stime desc");
                        break;
                    case 'Strain':
                        $com = strtolower($component);
                        $com = "dd_str_" . $com;
                        ...
                        break;
                    case 'EDM':
                        $condition = $this->getAttribute($code);
                        $result = mysql_query("select distinct b.dd_edm_time, b.dd_edm_line$cc from dd_edm b where $condition and b.dd_edm_pubdate <= now()  order by b.dd_edm_time desc");
                        break;
                    case 'Angle':
                        $condition = $this->getAttribute($code);
                        switch ($component) {
                            case 'Hort1':
                                $result = mysql_query("select distinct b.dd_ang_time, b.dd_ang_hort1$cc from ds a, dd_ang b where $condition and a.ds_pubdate <= now() and b.dd_ang_pubdate <= now() order by b.dd_ang_time desc");
                                break;
                            case 'Hort2':
                                $result = mysql_query("select distinct b.dd_ang_time, b.dd_ang_hort2$cc from ds a, dd_ang b where $condition and a.ds_pubdate <= now() and b.dd_ang_pubdate <= now() order by b.dd_ang_time desc");
                                break;
                            case 'Vert1':
                                $result = mysql_query("select distinct b.dd_ang_time, b.dd_ang_vert1$cc from ds a, dd_ang b where $condition and a.ds_pubdate <= now() and b.dd_ang_pubdate <= now() order by b.dd_ang_time desc");
                                break;
                            case 'Vert2':
                                $result = mysql_query("select distinct b.dd_ang_time, b.dd_ang_vert2$cc from ds a, dd_ang b where $condition and a.ds_pubdate <= now() and b.dd_ang_pubdate <= now() order by b.dd_ang_time desc");
                                break;
                        }
                        break;
                    case 'GPS':
                        ...
                        switch ($component) {
                            case 'Lat':
                                $result = mysql_query("select distinct b.dd_gps_time, b.dd_gps_lat$cc , b.dd_gps_lon from ds a, dd_gps b where $condition and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                ...
                                break;
                            case 'Lon':
                                $result = mysql_query("select distinct b.dd_gps_time, b.dd_gps_lon$cc , b.dd_gps_lat from ds a, dd_gps b where $condition and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                $additionalData = mysql_query("select distinct min(b.dd_gps_time), b.dd_gps_lat, b.dd_gps_lon from ds a, dd_gps b where $condition and a.ds_pubdate <= now() and b.dd_gps_pubdate <= now()");
                                break;
                            case 'Elev':
                                $result = mysql_query("select distinct b.dd_gps_time, b.dd_gps_elev$cc from ds a, dd_gps b where $condition and b.dd_gps_pubdate <= now() group by b.dd_gps_time order by b.dd_gps_time desc");
                                break;
                        }
                        break;
                    case 'GPSVector':
                        if ($component == "dmag")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_dmag $cc from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() order by b.dd_gpv_stime desc");
                        else if ($component == "daz")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_daz $cc from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() order by b.dd_gpv_stime desc");
                    ...
                    case 'Leveling':
                        $condition = $this->getAttribute($code);
                        $query = "select b.dd_lev_time, b.dd_lev_delev$cc from ds a, dd_lev b where $condition and b.dd_lev_pubdate <= now() order by b.dd_lev_time desc";
                        $result = mysql_query($query);
                        break;
                }
                break;
        ...
        if ($numberOfRows > 50000) {
            ...
            while ($array = mysql_fetch_array($result, MYSQL_NUM)) {
                ...
                if($table=="Interval") {
                    array_push($data[0][$count-1],$array[5]);
                }
            }
