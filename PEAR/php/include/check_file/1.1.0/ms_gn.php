...
if (!v1_set_owners($ms_gn_obj)) {
	...
	$errors[$l_errors]['message']="&lt;".$ms_gn_name." code=\"".$code."\"&gt; is missing information: please specify owner";
	$l_errors++;
	return FALSE;
}
...
if (!empty($ms_gn_stime) && !empty($ms_gn_etime)) {
	if (strcmp($ms_gn_stime, $ms_gn_etime)>0) {
		...
		$errors[$l_errors]['message']="In &lt;".$ms_gn_name." code=\"".$code."\"&gt;, start time (".$ms_gn_stime.") should be earlier than end time (".$ms_gn_etime.")";
		$l_errors++;
		return FALSE;
	}
}
...
if (!v1_check_dupli_timeframe($ms_gn_name, $ms_gn_key, $code, $ms_gn_stime, $ms_gn_etime, $final_owners, $dupli_error)) { 
	// Duplication found
	$errors[$l_errors]=array();
...
if (!v1_check_db_cn($ms_gn_name, $ms_gn_key, $code, $ms_gn_stime, $ms_gn_etime, $final_owners, $check_db_error)) {
	// Duplication found
	$errors[$l_errors]=array();
...
foreach ($ms_gn_obj['value'] as &$ms_gn_ele) {
	switch ($ms_gn_ele['tag']) {
		case "VOLCANOES":
			...
			v1_record_obj($ms_gn_key, $code, $data);
			break;
