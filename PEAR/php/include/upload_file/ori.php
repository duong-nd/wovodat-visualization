...
if ($file_type=="ori_no_ul") {
	...
	if (!file_record($ori_file_name, "P", $file_type, $current_time_iso, $cc_id_load, $record_error)) {
		...
		$_SESSION['errors'][0]['message']=$record_error." [ori.php -> file_record(ori_file_name=$ori_file_name, P)]";
		$_SESSION['l_errors']=1;
		header('Location: '.$url_root.'system_error.php');
		exit();
	}
	...
	if (!rename($undo_file, $move_undo_file_name)) {
		$_SESSION['errors'][0]=array();
		$_SESSION['errors'][0]['code']=1151;
		$_SESSION['errors'][0]['message']="ori.php -> rename(undo_file=".$undo_file.", move_undo_file_name=".$move_undo_file_name.")]";
		$_SESSION['l_errors']=1;
		header('Location: '.$url_root.'system_error.php');
		exit();
	}
