...
// get the station code
                    $code = $temp[0];
// sd_ivl
                    $value = mysql_query("select a.ss_id from sd_ivl a, ss b where a.ss_id = b.ss_id and b.ss_code = '$code'");
...
// hd
                    ...
                    if ($value && mysql_num_rows($value)) {
                        ...
                        $this->chkNull("Hydrologic&Data&$code&$temp[1]&$temp[2]&Comp;");
                    }
                }
                break;
            case 'thermal':
            ...
            case 'seismic':
                switch ($table) {
                    ...
                    case 'EVS':
                        ...
                        $que="select addtime(b.sd_evs_time, b.sd_evs_time_ms), b.$field $cc, IFNULL(b.sd_evs_eqtype,'Undefined') as eqtype from ss a, sd_evs b where b.$field is not null and a.ss_code = '$code' and a.ss_id = b.ss_id and a.ss_pubdate<=now() and b.sd_evs_pubdate<=now() $limitStr";
                        $result = mysql_query($que);
                        break;
                    case 'INT':
                        $field="";
                    ...
                    case 'TRM':
                        ...
                        $que="select b.sd_trm_stime, b.$field $cc, b.sd_trm_type as trm_type, b.sd_trm_qdepth as qdepth, b.sd_trm_etime as etime, IFNULL(b.sd_trm_type,'Undefined') as trm_type, IFNULL(b.sd_trm_qdepth,'Undefined') as qdepth from ss a, sd_trm b where a.ss_code='$code' and b.$field is not null and ((b.ss_id is not null and b.ss_id=a.ss_id) or (b.sn_id is not null and b.sn_id=a.sn_id)) and a.ss_pubdate<=now() and  b.sd_trm_pubdate<=now() $limitStr";
                        $result = mysql_query($que);
...
where a.ds_code = '$code' and a.ds_pubdate <= now() and b.dd_tlt_pubdate <= now() $limitStr");
                    ...
                    case 'Strain':
                        ...
                        $result = mysql_query("select b.dd_str_time, b.$attribute $cc from ds a, dd_str b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_str_pubdate <= now() $limitStr");
                    ...
                    case 'GPSVector':
                        if ($component == "dmag")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_dmag $cc , b.dd_gpv_etime as etime from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() $limitStr");
                        else if ($component == "daz")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_daz $cc , b.dd_gpv_etime as etime  from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() $limitStr");
                        else if ($component == "NS")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_N $cc , b.dd_gpv_etime as etime  from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() $limitStr");
                        else if ($component == "EW")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_E $cc , b.dd_gpv_etime as etime  from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() $limitStr");
                        else if ($component == "Z")
                            $result = mysql_query("select b.dd_gpv_stime, b.dd_gpv_vert $cc , b.dd_gpv_etime as etime  from ds a, dd_gpv b where a.ds_code = '$code' and a.ds_id = b.ds_id and a.ds_pubdate <= now() and b.dd_gpv_pubdate <= now() $limitStr");
                        break;
                    case 'Leveling':
                        $condition = $this->getAttribute($code);
                        $query = "select b.dd_lev_time, b.dd_lev_delev$cc from ds a, dd_lev b where $condition and b.dd_lev_pubdate <= now() $limitStr";
                        $result = mysql_query($query);
                        break;
                }
                break;
            case 'field':
                switch ($table) {
                    case 'ElectricFields':
                        $result = mysql_query("select b.fd_ele_time, b.fd_ele_field$cc from fs a, fd_ele b  where a.fs_code = '$code' and a.fs_code = b.fs_code and a.fs_pubdate <= now() and b.fd_ele_pubdate <= now() $limitStr");
                        break;
                    case 'Gravity':
                        $result = mysql_query("select b.fd_gra_time, b.fd_gra_fstr$cc from fs a, fd_gra b where a.fs_code = '$code' and a.fs_code = b.fs_code and a.fs_pubdate <= now() and b.fd_gra_pubdate <= now() $limitStr");
                        break;
                    case 'MagneticFields':
                        $result = mysql_query("select b.fd_mag_time, b.fd_mag_f$cc from fs a, fd_mag b where a.fs_code = '$code' and a.fs_code = b.fs_code and a.fs_pubdate <= now() and b.fd_mag_pubdate <= now() $limitStr");
                        break;
                    case 'MagneticVector':
                        if ($component == "Dec")
                            $result = mysql_query("select b.fd_mgv_time, b.fd_mgv_dec$cc from fs a, fd_mgv b where a.fs_code = '$code' and a.fs_code = b.fs_code and a.fs_pubdate <= now() and b.fd_mgv_pubdate <= now() $limitStr");
                        else if ($component == "Incl")
                            $result = mysql_query("select b.fd_mgv_time, b.fd_mgv_incl$cc from fs a, fd_mgv b where a.fs_code = '$code' and a.fs_code = b.fs_code and a.fs_pubdate <= now() and b.fd_mgv_pubdate <= now() $limitStr");
                ...
                switch ($table) {
                    ...
                    case 'SoilEfflux':
                        ...
                        if($field=="gd_sol_tflux") {
                            $result = mysql_query("select b.gd_sol_time, b.$field $cc, b.sol_species as gas_species from gs a, gd_sol b where a.gs_code ='$code' and b.gs_id = a.gs_id and a.gs_pubdate <= now() and b.gd_sol_pubdate <= now() $limitStr");
                        } else
                            $result = mysql_query("select b.gd_sol_time, b.$field $cc from gs a, gd_sol b where a.gs_code ='$code' and b.gs_id = a.gs_id and a.gs_pubdate <= now() and b.gd_sol_pubdate <= now() $limitStr");
            ...
            case 'hydrologic':
                ...
                $result = mysql_query("select b.hd_time, b.$attribute $cc from hs a, hd b where a.hs_code = '$code' and a.hs_id = b.hs_id and a.hs_pubdate <= now() and b.hd_pubdate <= now() $limitStr");
                break;
            case 'thermal':
                $attribute = 'td_temp';
                switch($table) {
                    case 'Data':
                        ...
                        $result = mysql_query("select b.td_time, b.$field $cc from ts a, td b where a.ts_code = '$code' and a.ts_id = b.ts_id and a.ts_pubdate <= now() and b.td_pubdate <= now() $limitStr");
                        break;
                    case 'ThermalImage':
                        $map = array(
            ...
            case 'meteo':
                ...
                $more = "";
                if($component=='prec') $more = ' , med_tprec as tprec ';
                $result = mysql_query("select b.med_time, b.$attribute $cc $more from ms a, med b where a.ms_code = '$code' and a.ms_id = b.ms_id and a.ms_pubdate <= now() and b.med_pubdate <= now() $limitStr");
    ...
    private function getEQTypeLabel($type) {
        $que="select st_eqt_name as name from st_eqt where st_eqt_org='$type'";
        $result=mysql_query($que);
